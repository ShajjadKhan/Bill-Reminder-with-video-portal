...
function openCollectionModal(id, name) {
    ...
    fetch('get_unpaid.php?cid=' + id).then(r => r.json()).then(data => {
        ...
        data.forEach((m, idx) => {
            ...
            html += '<td><input type="number" name="amounts[]" class="form-control amount-input" value="' + m.due + '" step="1" onchange="updateTotal()" style="width:120px"></td>';
            html += '</tr>';
        });
        html += '</tbody></table>';
